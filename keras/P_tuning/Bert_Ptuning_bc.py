...
if type_m == "S":
    ...
    # 加载数据集
    train_data = load_data('./sentiment/sentiment/sentiment.train.data')
    valid_data = load_data('./sentiment/sentiment/sentiment.valid.data')
    test_data = load_data('./sentiment/sentiment/sentiment.test.data')
else:
    dict_labels = {'其他口号': 0, '工作时间': 0, '要求': 0, '描述': 0, '薪资福利': 0, '公司介绍': 0, '工作地址': 0, '岗位晋升/职业发展': 0, '联系方式': 0,'职位名称':1}
    ...
    train_data, test_data, valid_data = load_data(r'data/corpus.xlsx',sheet_name = '按顺序1000')
...
if type_m == 'S':
    ...
    pos_id = tokenizer.token_to_id(u'其')
    neg_id = tokenizer.token_to_id(u'职')
...
class data_generator(DataGenerator):
    ...
    def __iter__(self, random=False):
        ...
        for is_end, (text, label) in self.sample(random):
            print('is_end:',is_end, 'text:',text, 'label:',label)
